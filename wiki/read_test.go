...
func TestGetPagesBatch_Success(t *testing.T) {
	server := mockMediaWikiServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		if action == "query" {
			...
			// Check that titles are pipe-separated
	...
	// Check that we got content for existing pages
	foundPage1 := false
	foundMissing := false
...
func TestGetPagesBatch_BatchSizeLimit(t *testing.T) {
	...
	// Create 60 titles (more than MaxBatchSize)
	...
	// Result should reflect only MaxBatchSize titles
